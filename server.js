document.addEventListener("DOMContentLoaded", () => {
  ...
  let gameStarted = false;     // Flag para marcar que el juego ya inició
  ...
  /* --------------------------
     DIBUJAR ROSCO
     Se usa containerSize = 400 y radius = 170
  -------------------------- */
  function drawRosco() {
    roscoContainer.innerHTML = "";
    const total = questions.length;
    const containerSize = 400;  // Debe coincidir con el max-width en CSS
    const radius = 170;
  ...
  function showQuestion() {
    if (!gameStarted) return; // Solo proceder si el juego ha iniciado
    if (queue.length === 0) {
      endGame();
      return;
    }
  ...
  /* --------------------------
     VALIDAR RESPUESTA
     Se ignoran respuestas vacías para evitar conteo accidental
  -------------------------- */
  function checkAnswer() {
    if (!gameStarted) return;
    // Si no se ingresa respuesta alguna, no cuenta (evita error si se presiona Enter sin texto)
  ...
  /* --------------------------
     MOSTRAR MODAL DE PÉRDIDA (al alcanzar 3 errores)
  -------------------------- */
  ...
  // Cargar las preguntas desde el servidor y esperar a que el usuario inicie el juego
  loadQuestions();
});
